...
import { ARBITRARY_BRACKET_RE, ARBITRARY_CURLY_RE, ARBITRARY_PAREN_RE, ENV_PREFIX, ENV_PREFIX_LENGTH } from './arbitrary.const';
...
function textFromValue(v: string): Record<string, string> {
  const t = v.trim();
  if (/^(#|rgb|hsl|color-mix|var\(|oklch|env\()/i.test(t) || t === 'currentColor' || t === 'inherit' || t === 'transparent') {
    return { color: t };
  }
  return { 'font-size': t };
}
...
function bgFromValue(v: string): Record<string, string> {
  const t = v.trim();
  if (/^(#|rgb|hsl|linear-gradient|radial-gradient|url\(|var\(|oklch|env\()/i.test(t)) {
...
const BOX_SHORTHAND_PARTS_4 = 4;
const BOX_SHORTHAND_PARTS_3 = 3;
const BOX_SHORTHAND_PARTS_2 = 2;

function expandBoxShorthand(raw: string): string {
  const t = raw.trim();
  const parts = t.split(/[\s_]+/).filter(Boolean);
  if (parts.length === BOX_SHORTHAND_PARTS_4) {
    return `${parts[0]} ${parts[1]} ${parts[2]} ${parts[3]}`;
  }
  if (parts.length === BOX_SHORTHAND_PARTS_3) {
    return `${parts[0]} ${parts[1]} ${parts[2]}`;
  }
  if (parts.length === BOX_SHORTHAND_PARTS_2) {
...
function applyPrefix(prefix: string, value: string): Record<string, string> | null {
  ...
  const map: Record<string, () => Record<string, string>> = {
    ...
    content: () => ({ content: v }),
    'line-height': () => ({ 'line-height': v }),
    'max-lines': () => ({ '-webkit-line-clamp': v }),
    'container-type': () => ({ 'container-type': v }),
    'container-name': () => ({ 'container-name': v }),
  };
  const fn = map[prefix];
  return fn ? fn() : null;
}
...
export function parseArbitraryClass(className: string): ArbitraryParseResult | null {
  ...
  if (p) {
    ...
    if (varName.startsWith(ENV_PREFIX)) {
      const envVar = varName.slice(ENV_PREFIX_LENGTH).replace(/_/g, ' ');
      return { prefix: p[1], value: `env(${envVar})`, syntax: 'paren' };
    }
